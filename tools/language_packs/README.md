...
- Поле `glossRu` для **английского** дополняется встроенным словарём `data/en_ru_core.json` (редактируемый список пар).
- Для **иврита, греческого, арабского** в `glossRu` записывается пояснение-заготовка на русском (нативная форма в `display`/`lemma` из списков wordfreq). Для учебных целей рекомендуется последующая ручная или словарная вычитка.

Не используйте сгенерированные нейросетью глоссарии без проверки прав и фактологии.
